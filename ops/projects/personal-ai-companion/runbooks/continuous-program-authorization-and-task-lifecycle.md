...
in `ARCHITECTURE_TODO.md`; orders 8-12F are repeated here as the accepted local
dependency chain. Order 12G is the next selected local/default-off slice.
...
| 12G | `PAC-HEALTH-SHORTCUT-SCOPED-CREDENTIAL` (`selected; not implemented`) | L1 local/default-off source; deployment or real issuance is L3 | Add a dedicated opaque, domain-separated digest credential accepted only by `POST /v1/health/shortcut-analysis`, bound to the trusted owner and individually revocable. | Existing access/refresh tokens are forbidden in Shortcuts. No implementation, issuance, deployment, phone installation, or real health transmission is accepted yet. |
