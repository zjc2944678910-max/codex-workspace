...
| StackChan App/adapter integration | `field-confirmed; reliability tests added` | On 2026-07-13 the owner observed the bounded App-driven `happy -> correlated ACK -> neutral -> correlated ACK` LCD sequence; the final screen was `neutral`, queue depth was `0`, and source is retained through product commit `9dbfafc` in current `main@0e53f74`. Product `0e53f74` adds deterministic two-device replay and SQLite restart/expiry coverage without changing production Bridge/queue source. | This accepts only the LCD allowlist `happy` and synthetic reliability coverage. Repeat field reliability, continuous polling, and App paths for audio, motion, touch, camera, memory, or HealthKit remain unconfirmed and require new scope/gates. |
...
in `ARCHITECTURE_TODO.md`; orders 8-12H plus 9C are repeated here as the accepted
...
| 9C | `PAC-CUSTOM-PROVIDER-HTTP-EXECUTOR` (`completed 2026-07-16`, default-off) | L1 | Accepted at product `9ebda8c`; see the [manifest](../manifests/provider-http-executor-v0.1.md) and [acceptance report](../reports/provider-http-executor-v0.1-acceptance-20260716.md). | Caller-opt-in OpenAI-compatible executor with bounded HTTP and redaction rules; no default network, real Provider, credential, or deployment. |
