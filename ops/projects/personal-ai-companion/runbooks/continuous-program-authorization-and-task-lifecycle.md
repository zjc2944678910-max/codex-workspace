...
| Health sources | `local/mock verified` | Product `b6209a7` is the order-12 source-seam anchor; current product `main@7905b12` retains orders 12A-12F and adds the accepted 12G owner-bound credential scoped only to Shortcut analysis. The latest verification passed `10` dedicated credential tests, `283` combined health-chain tests, and `1698` full Python tests with the existing warning. | No paid membership is assumed. There is no real-device read, installed/executed Shortcut, real credential issuance/handoff, HTTP action, HTTPS/rate-limited deployment, real health data, third-party adapter, or authenticated cloud-chat health transmission. Collection and off-device transmission remain separate gates. |
| Memory/privacy | `local/mock verified` | Current product `main@7905b12` retains Phase 2/3 and default-off Phase 4A/4B source with synthetic/temp-store verification. | Authenticated cloud chat disables candidate writes. Raw private-data inspection/export, live owner binding, runtime data/schema change, real migration, vault wiring, retention execution, and hard deletion require separate scopes. |
...
in `ARCHITECTURE_TODO.md`; orders 8-12G are repeated here as the accepted local
dependency chain. No post-12G implementation slice is selected here.
