...
| Memory/privacy | `local/mock verified` | Product `main@ad18cd0` retains Phase 2/3 and default-off Phase 4A/4B source with synthetic/temp-store verification. | Authenticated cloud chat disables candidate writes. Raw private-data inspection/export, live owner binding, runtime data/schema change, real migration, vault wiring, retention execution, and hard deletion require separate scopes. |
| Optional integration contracts | `local/mock verified` | Product commit `65d47b5`, retained in `main@ad18cd0`, freezes strict provider, MCP, phone-action, and five-family health-source v0.1 DTOs with synthetic fixtures; `102` focused and `1299` full tests passed at that contract baseline. | The contracts do not connect MCP, phone actions, health adapters, real data, or live services. |
| Custom Provider registry core | `local/mock verified` | Product `main@ad18cd0` adds Provider-specific envelope encryption, versioned mode-`0600` SQLite persistence, redacted profiles, synthetic health, optimistic concurrency, metadata-only audit, and explicit `normal`-only fallback policy. Provider-focused `85`, pattern `136`, and full `1384` tests passed. | No KEK lifecycle, API route, router/client execution, network call, iOS settings, real provider, real endpoint, or real credential is accepted. Sensitive privacy classes fail closed. |
...
`PAC-DOCS-SYNC` precedes every implementation wave. Completed orders 1-9A are
