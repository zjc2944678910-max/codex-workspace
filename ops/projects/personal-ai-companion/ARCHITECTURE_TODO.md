...
- Clean remote committed baseline: the product default branch `main` is pushed
  at `0e53f74`; order 12 remains anchored at `b6209a7`, order 12A at `4a8b52e`,
  ...
  regression coverage. The final combined Python suite passes `1743` tests.
  ...
  and the additive iOS HealthSource adapter seam plus the transport-free
  owner-summary intake contract (36 focused intake tests; 1475 full Python
  tests), fixture-only manual-export normalization (43 focused tests; 103
  combined health tests; 1518 full Python tests), and the default-off
  off-device consent envelope (55 focused tests; 158 combined health tests;
  1573 full Python tests), the local qualitative analysis contract (65 focused
  tests; 223 combined health tests; 1638 full Python tests), and the explicitly
  injected owner Shortcut analysis API (16 focused tests; 349 combined
  Cloud/health tests; 1654 full Python tests), and the local owner Shortcut
  aggregation policy/reference recipe (34 focused tests; 273 combined health
  tests; 1688 full Python tests), and the dedicated 12G Shortcut credential
  boundary (10 credential tests; 283 combined health tests; 1698 full Python
  tests), and the 12H hardening/deployment source (38 focused tests; 1713 full
  Python tests), plus
  redacted backend/local iOS status/selection wiring. Product `c550d4b` adds
  independently default-off single-owner Provider management and the fixed
  read-only MCP call UI, plus revision-safe Provider deletion. Product
  `54a069a` adds explicit-confirmation Chat proposals for exactly six local
  AlarmKit/EventKit/MediaPlayer actions. Product
  `2dc2948` also carries
  the bounded local/mock MCP gateway slice for one read-only information tool,
  and the first fixed public iOS system-share action. The current baseline is
  `main@0e53f74` and carries those feature anchors plus the health-source seam,
  both fixture-only health input contracts, the separate off-device consent
  contract, the qualitative analysis contract, and its default-absent owner-only
  Shortcut analysis API, the local-only 12F aggregation policy/recipe, the
  ...
  passed the full Swift/App builds, the affected smoke set, Simulator and
  Personal Team device builds, device install/launch, synthetic visual QA, and
  `1711` full Python tests at product `54a069a`. No real native permission prompt
  or side effect was executed.
- Product direction update: Xiaoxin remains a single-owner personal app. Paid
  ...
  HealthKit remains optional implemented code whose signed-device/data behavior
  is unaccepted. The bounded
  custom-provider registry/runtime, authenticated owner API, synthetic route,
  and default-off redacted backend/local iOS owner-tool slice are accepted.
  Product `c550d4b` exposes Provider create/replace/delete/validation and the
  exact local read-only MCP call only behind independent default-off gates. The
  first local/mock MCP gateway slice is accepted at product `2dc2948`. Real
  provider transport/health probing, remote MCP servers, state-changing MCP
  execution remain planned but not yet implemented. The Python owner-summary
  and manual-export normalization contracts are accepted locally, while the
  owner Shortcut and Swift manual-export adapters remain inert; no external
  intake is enabled. The separate default-off off-device consent contract is
  accepted locally, but no health summary is transmitted and authenticated chat
  still rejects health context. The model-agnostic, non-diagnostic
  health-analysis contract is accepted locally at `c0cd301`; product `2360cba`
  adds an explicitly injected owner-authenticated API that composes the accepted
  intake, transfer-consent, and categorical analysis contracts. The route is
  absent without both its dedicated service and trusted owner resolver. It adds
  no provider/model execution, free-form narrative, persistence, chat, deployed
  composition, chart UI, token handoff, or iOS UI. Product `09b86c7` adds the
  fixed 21+7-day local aggregation policy and owner-buildable reference recipe;
  it does not install/run a Shortcut, read real samples, or call the API.
  Product `7905b12` adds the accepted dedicated revocable opaque credential
  scoped only to the health-analysis route, with digest-only persistence and
  individual revocation. Product `5225740` adds finite expiry, atomic rotation,
  credential/IP limits before body parsing, HMAC-derived metadata-only audit,
  bounded retention, strict pre-buffer request sizing, Alembic, and a
  default-off deployed factory. The additive schema and pre-closeout candidate
  are dark-deployed, but the route is disabled and the running image predates
  the final source repairs. It adds no real issuance/handoff, enabled HTTPS
  action, phone call, Swift, or iOS UI. The separate
  `PAC-IOS-OWNER-TOOLS-UI` slice is complete at `c550d4b`. The order-13
  `PAC-IOS-CONVERSATIONAL-NATIVE-ACTIONS` slice is complete at `54a069a`; no
  live or device slice is automatically selected. It adds only alarm creation,
  calendar-event creation, reminder creation, and music play/pause/next after
  explicit confirmation. Arbitrary targets, background automation, and
  catalog-wide MusicKit remain out of scope.
- Product closeout: the patch-equivalent `codex/pac-ios-product-polish` and the
  fully merged converged/worker local refs were retired after verification.
  Their scratch worktrees were detached without deleting uncommitted files.
  `codex/pac-healthkit-scope-copy` remains because it has ten commits not
  absorbed by `main`; it requires selective review rather than a blind merge.
- Last recorded live baseline: the 2026-07-16 dark deployment runs
  `xiaoxin-cloud-api:20260716T111947-health-dark`, retains native Google and
  Authentik, keeps built-in email/OTP disabled, applies Alembic `20260716_0001`,
  and keeps the health route disabled/404 with both new tables empty. The image
  predates product `5225740`'s final two source repairs. Health, storage,
  database, tunnel, capability, and rollback evidence are recorded in
...
- Authentication acceptance checkpoint: a 2026-07-14 relaunch after the live
  ...
  `main@0e53f74`, and the single orphan family was revoked under a verified
...
- StackChan screen pages: `表情` / `聆听` / `状态` are **local/mock verified**
  ...
  the 12 expression families and four-frame animations remain local App assets.
  They do not expand the real v0.1 device allowlist beyond `happy` and
  `neutral`.
...
- iOS product/UI: the three-task line superseded the earlier mock-visual queue.
  ...
  `0e53f74`, has Xiaoxin branding/icon, user-scoped chat-history and device-cover
...
- Health sources: the committed host remains scoped to read-only HealthKit
  ...
  accepted. The Python owner-summary, fixture-only manual-export normalizer, and
  default-off single-request off-device consent envelope, plus the
  fingerprint-bound qualitative analysis DTOs, are accepted local contracts.
  The envelope copies only complete qualitative trends and does not transmit
  them by itself; the analysis contract emits controlled codes and no model
  narrative. Product `2360cba` adds an explicitly injected owner-only API that
  composes those accepted contracts, but the route is absent from the default
  factory and has no deployed composition, token handoff, persistence,
  provider/model execution, chat, Swift, or iOS UI. Product `09b86c7` adds the
  pure local 21+7-day aggregation policy and owner-buildable recipe. Empty
  Shortcut results fail closed as missing, one stable source is required per
  family, and no real phone execution or HTTP call is accepted. Product
  `7905b12` adds the local/default-off 12G credential boundary: the composed
  analysis route accepts only its dedicated owner-bound credential, persistence
  stores only the domain-separated digest and lifecycle metadata, and individual
  revocation is available. At that 12G anchor there was no rate limit or
  expiry/rotation. Product `5225740` later adds finite expiry, atomic rotation,
  pre-body credential/IP limits, HMAC-derived metadata-only audit, bounded
  retention, Alembic, strict pre-buffer sizing, and a default-off deployed
  composition. The running dark image predates its final two source repairs and
  remains route-off. There is still no real issuance/handoff, phone
  storage/action, enabled HTTPS action, or real health transmission.
  Owner-run Shortcut/webhook and the Swift manual Apple Health
  export source remain owner-initiated, disabled-by-default seams with typed
  unavailable adapters; no `export.xml`/ZIP/file intake exists, and selected
  third-party adapters remain later work.
- Xiaoxin cloud/auth: the current recorded image is
  `xiaoxin-cloud-api:20260716T111947-health-dark`. It includes Alembic
  `20260716_0001`, but predates product `5225740`'s final two source repairs, so
  the health route remains disabled/404. Native Google is preferred, Authentik
  PKCE remains available as fallback, and SMS OTP remains disabled.
...
- Native Google Sign-In: Google iOS client `小芯 iOS` exists for Bundle ID
  ...
  active. Commit `b8462a9`, retained in current `main@0e53f74`, keeps pending
  markers for every non-`204` response, including `401` and unexpected `202`. The one orphan
  family was separately
  revoked under backup; no cross-owner merge or data move occurred.
- Memory/privacy: product `main` is pushed at `0e53f74` and retains the
...
- Memory runtime metadata phase 3: product commits `f758875`, privacy repair
  ...
  Those commits are retained in current `main@0e53f74`; the old branch is not
  current authority.
...
- Memory integration boundary: source present by feature landing `9dbfafc` and
  retained in current canonical `0e53f74` includes the synthetic Approved
...
Rows 1-9C retain dependency history: completed source/deployment slices plus
explicitly separate, still-manual checkpoints such as same-account re-login and
  Personal Team acceptance. They are not all fully complete. The owner-summary,
  manual-export normalization, off-device consent, qualitative analysis,
  explicitly injected owner Shortcut analysis API, local Shortcut
  aggregation/recipe, route-scoped credential, owner-tools, and conversational
  native-action slices are complete. The local retention-summary, Provider HTTP
  executor, and StackChan reliability slices are also complete, with the latter
  two remaining default-off/tests-only. No post-native-actions live item is
  selected; no deployment is authorized.
...
| --- | --- | --- | --- | --- |
| 0 | `PAC-DOCS-SYNC` (`completed 2026-07-16`) | L1 | Completed for this planning wave; repeat before each later implementation wave. | Current authority/contract corrections and historical supersession banners only; no product or live-state change. |
| 1 | `PAC-MEMORY-LOCAL-INGEST-SEAM` (`completed`, canonical) | L1 | Memory slice anchored at `934cec1` and retained in current canonical `0e53f74`; synthetic fixtures and temporary SQLite only. | Committed memory candidate/review/privacy state machine; no automatic ordinary-chat extraction or real data. |
...
| 1e | `PAC-MEMORY-RETENTION-SUMMARY` (`completed 2026-07-16`, local metadata only) | L1 | Accepted and pushed at product `3448e84`; see the [manifest](manifests/memory-retention-summary-v0.1.md) and [acceptance report](reports/memory-retention-summary-v0.1-acceptance-20260716.md). | Owner-only export exposes active/expired/no-expiry/suppressed aggregates from payload-free metadata; optional metadata failures degrade safely. No retention executor, deletion, vault, schema, or live data. |
...
| 7A | `PAC-STACKCHAN-QUEUE-RELIABILITY` (`completed 2026-07-16`, tests-only) | L1 | Accepted and pushed at product `0e53f74`; see the [manifest](manifests/stackchan-queue-reliability-v0.1.md) and [acceptance report](reports/stackchan-queue-reliability-v0.1-acceptance-20260716.md). | Deterministic two-device v0.2 replay and SQLite restart/expiry coverage; `90` StackChan tests and `1743` full Python tests. No production queue change, hardware, socket, or field reliability claim. |
...
| 9C | `PAC-CUSTOM-PROVIDER-HTTP-EXECUTOR` (`completed 2026-07-16`, default-off) | L1 | Accepted and pushed at product `9ebda8c`; see the [manifest](manifests/provider-http-executor-v0.1.md) and [acceptance report](reports/provider-http-executor-v0.1-acceptance-20260716.md). | Strict caller-opt-in OpenAI-compatible executor with bounded timeout/response, HTTPS/path/media validation, no redirects, and redacted errors; no app/runtime wiring or real network. |
| 10 | `PAC-MCP-GATEWAY-READONLY` (`completed 2026-07-15`) | L1 | Accepted at product `2dc2948`; see the [manifest](manifests/mcp-gateway-readonly-v0.1.md) and [acceptance report](reports/mcp-gateway-readonly-v0.1-acceptance-20260715.md). Product `main` and `origin/main` matched at that commit. | One local allowlisted read-only `server.local.context/context.today` tool with bounded timeout, input/output limits, concurrency, idempotency, metadata-only audit, and failure isolation; no remote server, state-changing tool, durable audit store, or arbitrary iOS MCP runtime. |
| 10A | `PAC-IOS-OWNER-TOOLS-UI` (`completed 2026-07-16`) | L1 local/synthetic, default-off | Accepted and pushed at product `c550d4b`; see the [manifest](manifests/ios-owner-tools-v0.1.md) and [acceptance report](reports/ios-owner-tools-v0.1-acceptance-20260716.md). Depends on 9B and 10. | Single-owner Provider create/replace/delete/validation UI and exact `server.local.context/context.today` call UI behind independent default-off gates; saved secrets never return to iOS, arbitrary MCP inputs remain unreachable, and no deployment or real Provider/MCP evidence is accepted. |
...
| 12A | `PAC-HEALTH-OWNER-SUMMARY-CONTRACT` (`completed 2026-07-15`) | L1 fixture-only | Accepted and pushed at product `4a8b52e`; see the owner-summary manifest and acceptance report. | Owner-authored compact JSON summary for the planned Shortcut/webhook source, strict fixed metadata, canonical five-family projections, deterministic snapshot conversion, and synthetic fixtures/tests only. No file read, network/HTTP transport, Shortcut invocation, cloud transmission, or iOS UI. |
| 12B | `PAC-HEALTH-MANUAL-EXPORT-NORMALIZATION` (`completed 2026-07-15`) | L1 fixture-only | Accepted and pushed at product `0665fd3`; see the [manifest](manifests/health-manual-export-normalization-v0.1.md) and [acceptance report](reports/health-manual-export-normalization-v0.1-acceptance-20260715.md). | Pre-aggregated synthetic mapping from five documented export markers to the canonical families, reusing snapshot validation/hash/freshness/conflict boundaries; `43` focused, `103` combined health, and `1518` full Python tests passed. No real file/ZIP/XML read, Swift adapter enablement, network, cloud transmission, or iOS UI. |
| 12C | `PAC-HEALTH-OFF-DEVICE-CONSENT-CONTRACT` (`completed 2026-07-15`) | L1 fixture-only privacy contract | Accepted and pushed at product `bff7398`; see the [manifest](manifests/health-off-device-consent-contract-v0.1.md) and [acceptance report](reports/health-off-device-consent-contract-v0.1-acceptance-20260715.md). | Literal `true` single-request transfer consent, fixed private-health purpose/destination class, five-minute lifetime, complete qualitative five-family envelope, content binding, and source/adapter stripping; `55` focused, `158` combined health, and `1573` full Python tests passed. No network/API route, model execution, persistence, authenticated-chat wiring, Swift source, or iOS UI. |
| 12D | `PAC-HEALTH-ANALYSIS-CONTRACT` (`completed 2026-07-15`) | L1 fixture-only | Accepted and pushed at product `c0cd301`; see the [manifest](manifests/health-analysis-contract-v0.1.md) and [acceptance report](reports/health-analysis-contract-v0.1-acceptance-20260715.md). Depends on 12C, not Personal Team signing or a provider. | Fingerprint-bound request/response DTOs with controlled five-family explanation codes, explicit qualitative uncertainty, and categorical chart metadata; `65` focused, `223` combined health, and `1638` full Python tests passed. No free-form narrative, numeric health value, provider/model execution, network/API route, persistence, authenticated-chat wiring, real health data, Swift source, or iOS UI. |
| 12E | `PAC-HEALTH-OWNER-SHORTCUT-ANALYSIS-API` (`completed 2026-07-15`) | L1 local/synthetic default-absent API | Accepted and pushed at product `2360cba`; see the [manifest](manifests/health-owner-shortcut-analysis-api-v0.1.md) and [acceptance report](reports/health-owner-shortcut-analysis-api-v0.1-acceptance-20260715.md). Depends on 12A, 12C, 12D, bearer auth, and a trusted owner resolver; it does not depend on Personal Team signing. | Strict 20 KiB owner-only `POST /v1/health/shortcut-analysis`, fixed five-minute consent window, non-cacheable exact 12D response, and no database write; `16` focused, `349` combined Cloud/health, and `1654` full Python tests passed. No default route mount, deployed composition, Shortcut recipe/token handoff, raw samples/export parser, persistence, replay store, model narrative, chat wiring, Swift, or iOS UI. |
| 12F | `PAC-HEALTH-OWNER-SHORTCUT-RECIPE` (`completed 2026-07-16`) | L1 local/synthetic aggregation + docs | Accepted and pushed at product `09b86c7`; see the [manifest](manifests/health-owner-shortcut-aggregation-v0.1.md) and [acceptance report](reports/health-owner-shortcut-aggregation-v0.1-acceptance-20260716.md). Depends on 12A and composes with 12E; it does not depend on Personal Team signing. | Fixed 21+7-day policy, 10/4/2/2 coverage, six-decimal half-up median bands, conservative empty/multi-source handling, lowercase UUID guidance, synthetic fixture, and owner-buildable recipe; `34` focused, `273` combined health-chain, and `1688` full Python tests passed. No installed/executed Shortcut, real sample read, credential, HTTP call, deployment, Swift source, or iOS UI. |
...
the target data, allowed store/schema reads and writes, backup/rollback
treatment, time window, and stop owner. The preflight does not itself authorize
a producer, store/schema change, planner invocation against real data,
...
Orders 9-13 and 12A above already cover provider, MCP, phone-action,
health-source, owner-summary, and Personal Team work; they are not duplicated
here as a second priority list.
