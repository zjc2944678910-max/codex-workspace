...
- Canonical product source `main` and `origin/main` are synchronized at
  ...
  current local privacy/documentation repair.
- The former order-14E StackChan worker and foreground result-polling work is no
  longer an uncommitted worktree: it was committed in `7f30745` and is retained
  by `7754727` together with persistent Chat memory/style work (`213b0b4`) and
  Provider/robot integration.
- The current local repair makes the custom Chat Provider boundary explicit:
  only prompts classified exactly `normal` may enter it; non-`normal` prompts
  and `owner_authorized_private_cloud_opus` routes bypass it and use the
  configured Relay. The repair is source-only and does not change a live image,
  Provider registry, credential, Bridge, device, or public endpoint.
- Product source authority and live deployment authority are separate. The
  [deployment ledger](DEPLOYMENT_LEDGER.md) remains authoritative for the last
  verified live image; a newer source commit is not evidence of deployment.
- See the [2026-07-20 local acceptance report](reports/provider-privacy-and-source-reconciliation-20260720.md)
  for the exact changed files, verification, residual risks, and rollback
  boundary.

### Historical Baseline (superseded source snapshot from 2026-07-17)

- Historical status: Mixed local/live, single-owner personal product. Local product
...
### Current Guardrails And Next Work
...
- Historical next-work note from 2026-07-17: the temporary authenticated Chat vertical is accepted. The
...
| --- | --- | --- |
| Product source | Product `HEAD`, `main`, and `origin/main` resolve to `7754727` before the current local privacy/documentation repair. Order 14E is committed in `7f30745` and retained by `7754727`; it is not an uncommitted field patch. | The current local repair is not committed or deployed. Newer source is never proof of a newer live image. |
| Realtime conversation v1 | The authorized 2026-07-21 repair installed the separate CoreS3 PCM worker and dedicated NVS credential, enabled its parallel boot path beside the unchanged v0.2 status daemon, installed/launched a signed iPhone Host, migrated the protected local memory database from registered v2 to v3 after backup, and brought up an authenticated Mac WebSocket service whose health reports both robot-audio and presence lanes. The physical path passed reconnect and continuous Chinese PCM: 56/56 100 ms slices, 133 ms maximum device interval, and zero intervals over 150 ms. A VAD-shaped synthetic real-provider turn completed Azure SDK STT -> warmed local 2B stream -> Azure SDK PCM TTS -> CoreS3 at 1686.7 ms to first audio and 3609.3 ms total; a speaking turn returned robot cancel ACK plus `turn.interrupted` in 56.9 ms with zero stale deltas. | Actual-room iPhone microphone/AEC and owner-spoken barge-in remain unconfirmed. Voiceprint calibration and LaunchAgent bootstrap remain pending, and a fresh signed rebuild is blocked until Xcode re-authenticates the existing Team account. The old v0.2/WAV path remains the rollback. |
| Checked-in iOS Host | The formal Host explicitly enables authenticated Chat, Provider management, and the bounded StackChan status path. It contains one local/private Bridge endpoint and one non-secret status device ID; the Bridge credential remains Keychain-only. `XiaoxinMCPEnabled` is absent. | These source settings do not authorize a new field session, unattended operation, public endpoint, Provider request, or deployment. |
| Provider / cloud image | The deployment ledger records `xiaoxin-cloud-api:20260717T2150-provider-probe` as the current API image, with 14 Provider profiles and `deepseek-chat` selected. Provider probe and main Chat request counts for that acceptance remained `0`. | Real Provider compatibility and a successful owner-triggered Provider request remain unconfirmed. The current privacy-classification repair is source-only. |
| Authenticated Chat, memory, and style | The 2026-07-17 ledger records standard persisted Chat, explicit candidate-memory opt-in, review-only/unpromoted candidates, and read-only `girlfriend_style` injection as live and server-side accepted. | Automatic extraction without literal opt-in, auto-promotion, authoritative retention execution, hard deletion, and private/sensitive custom-Provider routing remain blocked or unconfirmed. |
| StackChan | Dated field reports confirm the bounded v0.1 expression path, negotiated v0.2 hardware checks, and the read-only status path. The signed status roundtrip returned the queue to zero. | No unattended actuation, model-generated command execution, expanded v0.1 LCD allowlist, or repeated end-to-end reliability is authorized. |
| Health, MCP, and native actions | The last recorded live Health route remains disabled/404 with zero Health credential/audit rows. MCP remains an exact local read-only source capability whose iOS gate is absent. Native phone actions require local proposal plus explicit confirmation. | No real Health transfer, remote/state-changing MCP, arbitrary phone action, or accepted real native side effect is recorded. |

### Historical Detailed Evidence Index (through 2026-07-17)
...
| Provider, MCP, and phone-app integrations | Product `65d47b5` freezes the strict contracts, `ad18cd0` adds the encrypted registry, `295687f` adds the bounded owner/runtime API, and `2dc2948` adds exactly `server.local.context/context.today`. Product `c550d4b` adds default-off single-owner Provider management and the exact read-only MCP call UI. Product `9ebda8c` adds a caller-opt-in OpenAI-compatible HTTP executor with HTTPS, redirect, size, JSON-shape, and redacted-error boundaries. The current uncommitted local source slice adds an independently default-off authenticated Chat bridge: the existing iOS Provider form accepts URL/API credential once, the owner selection is persisted in the server registry, main Chat reads only the server-side profile ID, default fast/complex Opus routing remains distinct when configured, and Provider failure has one relay fallback with circuit isolation. Product `7547d8a` adds the fixed public share-sheet handoff, and `54a069a` adds confirmed-before-execution local AlarmKit/EventKit/MediaPlayer actions. See the [Provider HTTP acceptance](reports/provider-http-executor-v0.1-acceptance-20260716.md), [native-action acceptance](reports/ios-conversational-native-actions-v0.1-acceptance-20260716.md), and [local Chat Provider wiring report](reports/custom-provider-chat-wiring-20260717.md). | No real Provider/endpoint/credential, trusted live health probe, live reconfiguration, deployment, or sensitive privacy class is accepted. The checked-in bridge and iOS Provider gates are off and the current live image remains relay-only. The phone cannot run arbitrary MCP servers or receive stored secrets, and no real native side effect was used for acceptance. |
