...
- Current status: Mixed local/live, single-owner personal product. The product
  source of truth and GitHub default branch are `main` at `bff7398`; redundant
  ...
  consent contract accepted at product `bff7398`.
  ...
  is retained in current `main@bff7398`, and the single orphan family was
...
- Next useful work: the health-source boundary, fixture-only
  ...
  `PAC-HEALTH-OFF-DEVICE-CONSENT-CONTRACT` are accepted after the
  ...
  model, or expose iOS UI. The selected next local candidate is
  `PAC-HEALTH-ANALYSIS-CONTRACT`: a model-agnostic, non-diagnostic synthetic
  request/response boundary without network, provider execution, persistence,
  authenticated-chat wiring, or iOS UI.
...
retained in current canonical `main` at `bff7398`, together with a source-gated,
