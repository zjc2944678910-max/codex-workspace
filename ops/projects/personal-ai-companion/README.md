...
- Current status: Mixed local/live, single-owner personal product. The product
  source of truth and GitHub default branch are `main` at `0665fd3`; redundant
  ...
  intake contract accepted at product `4a8b52e`, and fixture-only manual-export
  normalization accepted at product `0665fd3`.
  ...
  is retained in current `main@0665fd3`, and the single orphan family was
...
- Next useful work: the health-source boundary, fixture-only
  `PAC-HEALTH-OWNER-SUMMARY-CONTRACT`, and fixture-only
  `PAC-HEALTH-MANUAL-EXPORT-NORMALIZATION` are accepted after the
  ...
  target completion. Both health input contracts are synthetic and
  transport-free; they do not read a file, open a network connection, invoke a
  Shortcut, or expose iOS UI. The selected next local candidate is
  `PAC-HEALTH-OFF-DEVICE-CONSENT-CONTRACT`: a separate default-off transfer
  decision and strict trend-only envelope, without network or model execution.
  The Personal Team device gate is optional/deferred and is not this task's
  dependency.
...
retained in current canonical `main` at `0665fd3`, together with a source-gated,
