...
| Memory/privacy | Product checkout `121334c` remains the current locally verified MVP (`435` focused tests). Branch-only successor `4a3a7df`, based on `3019a8c`, adds an explicit synthetic review-turn seam: narrow deterministic preference normalization, fail-closed opt-out/credential/identity/ambiguity/length handling, non-raw sensitive review, owner-confirmed promotion, deterministic duplicate/conflict signals, and scoped recall. It passed `59` focused and `975` full Python tests; see [the current memory-layer status](reports/memory-layer-current-status-20260711.md). | `4a3a7df` is not in the current checkout and is not live integration. No real/private database or data path was inspected or validated. Automatic ordinary-chat extraction, broad normalization, semantic/vector retrieval, automatic conflict resolution, vault-at-rest store wiring, an authoritative retention producer/executor, hard deletion, runtime authentication/admin UI, and NAS/HealthKit/cloud authorization remain unimplemented or unconfirmed. Dirty iOS product-polish work is excluded. |
