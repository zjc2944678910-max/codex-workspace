...
- Current status: Mixed local/live product. The remote canonical product branch
  `codex/initial-private-publish` is pushed at `9dbfafc`. It retains the Xiaoxin
  ...
  entry after relaunch through the restored session. Original-account and
  historical-data continuity and the forced-refresh/logout matrix remain
  unconfirmed. Dated deployment evidence is not proof of current live health.
...
- Next useful work: Independently verify original-account and historical-data
  continuity, forced token refresh, remote invalidation/logout, and the full
  logout/re-login matrix. The bounded App-to-StackChan v0.1 LCD
...
`9dbfafc`, while Xiaoxin cloud/auth has separate dated live deployment evidence;
