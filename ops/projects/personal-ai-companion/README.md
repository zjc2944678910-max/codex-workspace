...
- Current status: Mixed local/live, single-owner personal product. The product
  source of truth and GitHub default branch are `main` at `b6209a7`; redundant
  ...
  additive health-source seam accepted at product `b6209a7`.
  ...
  builds, and an unsigned Simulator Host build at product `main@b6209a7`.
...
- Next useful work: the health-source boundary is now accepted after the
  ...
  target completion. The health slice keeps HealthKit optional and exposes
  inert owner-run Shortcut/webhook/manual-export fallback seams. The next
  candidate is the optional `PAC-PERSONAL-TEAM-DEVICE-ACCEPTANCE` manual gate.
