...
- Current status: Mixed local/live, single-owner personal product. The product
  source of truth and GitHub default branch are `main` at `ad18cd0`; redundant
  ...
  and the local encrypted custom-provider registry core.
  No live chat or optional-integration vertical has been accepted.
  ...
  is retained in current `main@ad18cd0`, and the single orphan family was
...
- Next useful work: design `PAC-CUSTOM-PROVIDER-RUNTIME-INTEGRATION` around a
  secret-safe runtime adapter and explicit KEK lifecycle, then add authenticated
  owner API plus redacted iOS status/selection using synthetic/mock providers.
  Keep sensitive privacy classes and real endpoints blocked. After that, add one
...
| iOS product | Product `main` and the remote default branch are pushed at canonical `ad18cd0`. Logout accepts only HTTP `204` as remote-revocation success and retains pending markers for `401`, `202`, and temporary failures. Authenticated chat is source-gated and off by default; when enabled, non-temporary turns persist atomically, temporary turns skip persistence, and expired bounded context is pruned at startup and periodically. The [2026-07-15 registry-core acceptance](reports/custom-provider-registry-v0.1-acceptance-20260715.md) passed `1384` Python tests at that exact commit with one existing warning; prior AppFlow smoke, Ruff, Swift App build, and Simulator Host build evidence remains valid. | No live authenticated-chat vertical or account deletion is accepted. The product is personal/single-owner; public onboarding, multi-tenant administration, TestFlight, and App Store distribution are not current goals. The 12 expressions are App-local preview assets only; they are not a 12-expression live-device protocol. |
...
| Provider, MCP, and phone-app integrations | Product `65d47b5` freezes the strict integration contracts. Product `ad18cd0` adds the local custom-provider registry core: Provider-specific envelope encryption, external KEK injection, exact SQLite schema, redacted profiles, synthetic health, optimistic concurrency, metadata-only audit, and explicit `normal`-only fallback policy. See the [registry manifest](manifests/custom-provider-registry-v0.1.md). | KEK lifecycle, authenticated provider API, router/client execution, network calls, iOS settings, real providers, and sensitive privacy classes remain unimplemented. MCP connections/execution/audit and general phone-app actions also remain unimplemented. Stored provider/MCP secrets must not be returned to iOS. Arbitrary app control, background cross-app automation, and reading messages/notifications remain impossible or out of scope. |
...
| Memory/privacy | Product `main` at `ad18cd0` retains Phase 2 canonical recall selection, Phase 3 nullable `fact_key`/`expires_at_ms` persistence and metadata-first disclosure, plus Phase 4A/4B explicit opt-in admission and candidate-write wiring through `78103de`. The ordinary-chat writer is default-off and fails closed without a trusted owner resolver. The authenticated cloud-chat route is also off by default and explicitly disables candidate writes; when enabled, non-temporary turns persist bounded conversation context, temporary turns skip persistence, and expired context is pruned at startup and periodically. The [current full verification](reports/custom-provider-registry-v0.1-acceptance-20260715.md) passed `1384` Python tests at the same baseline. See [the Phase 3 report](reports/memory-runtime-metadata-phase-3-20260713.md) and [Phase 4 readiness/update](reports/memory-phase-4-readiness-20260714.md). | This is committed product source, not a live authenticated-chat/long-term-memory integration or evidence of a real-database migration. Automatic extraction without explicit opt-in, production trusted-owner binding, semantic/vector retrieval, broad automatic conflict resolution, vault-at-rest store wiring, authoritative retention execution, hard deletion, memory runtime auth/admin UI, and real NAS/health-source/cloud memory authorization remain unimplemented or unconfirmed. |
...
- `personal_ai_companion.providers` is the accepted local registry core. It is
  not an authenticated API, model-route executor, network client, iOS surface,
  KEK lifecycle, or real-provider acceptance.
...
retained in current canonical `main` at `ad18cd0`, together with a source-gated,
