...
Options:
  --dev-agent <name>       surgical_fixer (default) or refactor_worker.
...
function buildSliceFiles(runRoot, options = {}, taskIds = {}) {
  ...
  const devAgent = String(options.devAgent || "surgical_fixer").trim() || "surgical_fixer";
...
async function appendSlice(options = {}) {
  ...
  const result = {
    ...
    next_action: `send ${path.join(runRoot, "agents", taskIds.devTaskId, "dev-brief.md")} to ${options.devAgent || "surgical_fixer"}`,
  };
